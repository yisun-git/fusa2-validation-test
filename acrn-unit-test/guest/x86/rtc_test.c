...
/**
 *  @brief Case name: When a vCPU attempts to write guest RTC target register, ACRN hypervisor shall ignore the write_001
 *
 * Summary: the value of the RTC target register has no change after writing this target register with different data
 *
 */
...
/**
 * @brief Case name: RTC_When a vCPU attempts to read guest RTC target register and the current guest RTC index is greater than BH,
 *  ACRN hypervisor shall write 0 to guest AL_001
 *
...
/**
 * @brief Case name: RTC When a vCPU attempts to write 2 bytes to guest RTC index register, ACRN hypervisor shall ignore the write_001
 *
 * Summary: RTC When a vCPU attempts to write 2 bytes to guest RTC index register, ACRN hypervisor shall ignore the write_001
 *
 */
...
/**
 * @brief Case name: RTC_When a vCPU attempts to read guest RTC target register and the current guest RTC index is BH, ACRN hypervisor shall write 6H to guest AL_001
 *
 * Summary: dump the value of AL after reading from RTC target register
 *
 */
...
void rtc_rqmid_24495_reading_rtc_target_register_and_current_guest_rtc_index_is_BH(void)
{
	...
	report("\t\t rtc_rqmid_24495_reading_rtc_target_register_and_current_guest_rtc_index_is_BH %d", reg_b_value == B_DEFAULT_VALUE,reg_b_value);
}


/**
 * @brief Case name: When a vCPU attempts to write guest RTC index register, ACRN hypervisor shall keep guest AL as the current guest RTC index_001
 *
...
void rtc_rqmid_24556_reading_rtc_AL_is_keeped_to_be_current_rtc_index_register(void)
{
	...
	report("\t\t rtc_rqmid_24556_reading_rtc_AL_is_keeped_to_be_current_rtc_index_register %d", reg8_value == YEAR_INDEX, reg8_value);
}


/**
 * @brief Case name: When a vCPU attempts to write 2 bytes to guest RTC target register, ACRN hypervisor shall ignore the write_001
 *
...
void rtc_rqmid_24558_writing_2bytes_to_target_register_register_has_no_change(void)
{
	...
	report("\t\t rtc_rqmid_24558_writing_2bytes_to_target_register_register_has_no_change %d", reg16_value == reg16_value_prev, reg16_value);
}


/**
 * @brief Case name: when a vCPU attempts to read guest RTC index register, ACRN hypervisor shall write 0H to guest AL_001
 *
...
static void print_case_list(void)
{
	...
	printf("\t\t Case ID:%d case name:%s\n\r", 24557u, "When a vCPU attempts to write guest RTC target register, ACRN hypervisor shall ignore the write_001");

	printf("\t\t Case ID:%d case name:%s\n\r", 24494u, "RTC When a vCPU attempts to read guest RTC target register and the current guest RTC index is greater than BH, ACRN hypervisor shall write 0 to guest AL_001");

	printf("\t\t Case ID:%d case name:%s\n\r", 24560u, "RTC When a vCPU attempts to write 2 bytes to guest RTC index register, ACRN hypervisor shall ignore the write_001");

	printf("\t\t Case ID:%d case name:%s\n\r", 24495u, "RTC_When a vCPU attempts to read guest RTC target register and the current guest RTC index is BH, ACRN hypervisor shall write 6H to guest AL_001");

	printf("\t\t Case ID:%d case name:%s\n\r", 24556u, "When a vCPU attempts to write guest RTC index register, ACRN hypervisor shall keep guest AL as the current guest RTC index_001");

	printf("\t\t Case ID:%d case name:%s\n\r", 24558u, "When a vCPU attempts to write 2 bytes to guest RTC target register, ACRN hypervisor shall ignore the write_001");

	printf("\t\t Case ID:%d case name:%s\n\r", 24387u, "when a vCPU attempts to read guest RTC index register, ACRN hypervisor shall write 0H to guest AL_001");
