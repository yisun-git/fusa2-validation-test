...
/**
 * @brief Case name: information leakage of SFR-08 ACRN hypervisor shall mitigate the L1TF variant affecting VMM_benchmark_001
 *
 * Summary: This part verifies L1D FLUSH cache on hypervisor, target is to get benchmark data
 *
 */
void infoleak_rqmid_33619_L1TF_variant_affecting_VMM_benchmark_001(void)
{
	...
	report("\t\t %s", tsc_average2 > (tsc_average1 - 3*tsc_stdevv1) && tsc_average2 < (tsc_average1 + 3*tsc_stdevv1), __FUNCTION__);
	return;
}
 ...
 *
 * Summary: ACRN hypervisor shall expose indirect branch restricted speculation(IBRS) to any VM in compliance with Chapter 2.4.1, SESCM.
 *
 */
 ...
 *
 * Summary: ACRN hypervisor shall expose speculative store bypass disable (SSBD) to any VM in compliance with Chapter 4.2.2 and 5.1, SESCM.
 *
 */
 ...
 *
 * Summary: ACRN hypervisor shall expose indirect branch predictor barrier(IBPB) to any VM in compliance with Chapter 2.4.3 and 5.4, SESCM.
 *
 */
void infoleak_rqmid_33874_IBPB_expose_001(void)
{
	...
	//u64 val = rdmsr(IA32_PRED_CMD_MSR);
	wrmsr(IA32_PRED_CMD_MSR, 0x1);
#if 0
	val = rdmsr(IA32_PRED_CMD_MSR);
	if ((val & 0x1) == 0)
	{
		report("\t\t %s", 0, __FUNCTION__);
		return;
...
int main(int ac, char **av)
{
	...
	infoleak_rqmid_33619_L1TF_variant_affecting_VMM_benchmark_001();
